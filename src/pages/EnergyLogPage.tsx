...
// "HH:MM" → minutes. Returns NaN for empty / malformed.
...
// Duration in hours from start → end, supporting overnight ranges.
function computeSleepHours(start: string, end: string): number {
...
export default function EnergyLogPage() {
  ...
  const formHours = computeSleepHours(formData.sleepStart ?? '', formData.sleepEnd ?? '');
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-700 mb-1">
                  ...
                  <span className="ml-2 text-xs font-normal text-zinc-500 font-mono">
                    {formHours > 0 ? `· ${formatDuration(formHours)}` : ''}
                  </span>
                </label>
                <div className="flex items-center gap-2">
                  <div className="flex-1 relative">
                    <Moon size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-zinc-400 pointer-events-none" />
                    <input
                      type="time"
                      required
                      value={formData.sleepStart}
                      onChange={e => setFormData({ ...formData, sleepStart: e.target.value })}
                      className="w-full border border-zinc-300 rounded-lg pl-8 pr-2 py-2 text-sm focus:ring-2 focus:ring-zinc-900 focus:border-zinc-900 outline-none"
                      title="Bedtime"
                    />
                  </div>
                  <span className="text-zinc-400 text-xs">→</span>
                  <div className="flex-1 relative">
                    <Sunrise size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-zinc-400 pointer-events-none" />
                    <input
                      type="time"
                      required
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-hidden">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-zinc-200">
            {energyLogs.map(log => {
              const hours = computeSleepHours(log.sleepStart, log.sleepEnd);
              return (
                  ...
                  <td className="px-6 py-4 text-zinc-600">
                    {log.sleepStart && log.sleepEnd ? (
                      <div className="flex flex-col">
                        <span className="font-mono text-xs text-zinc-700">{log.sleepStart} → {log.sleepEnd}</span>
                        <span className="text-[10px] text-zinc-500 font-mono">{formatDuration(hours)}</span>
                      </div>
                    ) : (
